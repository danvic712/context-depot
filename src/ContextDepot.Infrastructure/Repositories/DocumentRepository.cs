...
public sealed class DocumentRepository(ContextDepotDbContext db) : IDocumentRepository
{
    public Task<Document?> GetByIdAsync(Guid ownerId, Guid documentId, CancellationToken cancellationToken) =>
        db.Documents.AsNoTracking().Include(x => x.Chunks).SingleOrDefaultAsync(x => x.OwnerId == ownerId && x.Id == documentId, cancellationToken);
    ...
    public Task<Document?> GetByPathAsync(Guid ownerId, Guid workspaceId, string normalizedPath, CancellationToken cancellationToken) =>
        db.Documents.AsNoTracking().Include(x => x.Chunks).SingleOrDefaultAsync(x => x.OwnerId == ownerId && x.WorkspaceId == workspaceId && x.Path == normalizedPath, cancellationToken);

    public async Task MarkIndexPendingAsync(Guid ownerId, Guid workspaceId, string normalizedPath, DateTimeOffset now, CancellationToken cancellationToken)
    {
        var document = await db.Documents.SingleOrDefaultAsync(x => x.OwnerId == ownerId && x.WorkspaceId == workspaceId && x.Path == normalizedPath, cancellationToken);
    ...
    public async Task<DocumentReconcilePersistenceResult> ReconcileIndexAsync(DocumentIndexWrite write, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var document = await db.Documents.Include(x => x.Chunks).SingleOrDefaultAsync(x =>
                x.OwnerId == write.OwnerId && x.WorkspaceId == write.WorkspaceId && (x.Id == write.DocumentId || x.Path == write.Path), cancellationToken);
            ...
            document ??= new Document(write.DocumentId, write.OwnerId, write.WorkspaceId, write.Path, write.Title, write.Now);
            if (isNew)
            {
            ...
            {
                document.Chunks.Add(new DocumentChunk(chunk.Id, write.OwnerId, document.Id, write.WorkspaceId, chunk.Ordinal, chunk.HeadingPath, chunk.Content, chunk.ContentHash, write.Now));
            }
    ...
    public async Task<DocumentArchivePersistenceResult> ArchiveAsync(Guid ownerId, Guid documentId, DateTimeOffset now, CancellationToken cancellationToken)
    {
        await using var transaction = await db.Database.BeginTransactionAsync(IsolationLevel.Serializable, cancellationToken);
        var document = await db.Documents.SingleOrDefaultAsync(x => x.OwnerId == ownerId && x.Id == documentId, cancellationToken);
        if (document is null)
        {
