...
namespace ContextDepot.MCP.Documents;

public sealed class DocumentTools(ILogger<DocumentTools> logger)
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await MCPToolExecutor.ExecuteAsync(
            () => service.UpsertAsync(new UpsertDocumentCommand(workspace, path, title, content, expectedContentHash), cancellationToken),
            logger);
    }
